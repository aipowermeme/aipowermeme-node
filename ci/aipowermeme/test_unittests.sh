...
source ./ci/aipowermeme/matrix.sh
...
cd build-ci/aipowermeme-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_aipowermeme.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_aipowermeme --catch_system_errors=no -l test_suite
  else
