...
*Setup instructions for a Gitian build of AIPowerMeme Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AIPowerMeme
...
- [Building AIPowerMeme Core](#building-aipowermeme-core)
...
Clone the git repositories for AIPowerMeme Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aipowermeme
git clone https://github.com/aipowermemepay/gitian.sigs.git
```
...
in the AIPowerMeme Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building AIPowerMeme Core
----------------

To build AIPowerMeme Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the AIPowerMeme Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aipowermeme/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/aipowermeme
...
For example:
```bash
URL=https://github.com/crowning-/aipowermeme.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit aipowermeme=${COMMIT} --url aipowermeme=${URL} ../aipowermeme/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aipowermeme=${COMMIT} --url aipowermeme=${URL} ../aipowermeme/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aipowermeme=${COMMIT} --url aipowermeme=${URL} ../aipowermeme/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aipowermeme git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aipowermeme/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/aipowermeme
SIGPATH=/some/root/path/aipowermeme-detached-sigs

./bin/gbuild --url aipowermeme=${BTCPATH},signature=${SIGPATH} ../aipowermeme/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aipowermeme-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aipowermeme-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aipowermeme-osx-build.assert
```
...
[aipowermeme/gitian.sigs](https://github.com/aipowermemepay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
