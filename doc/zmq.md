...
The AIPowerMeme Core daemon can be configured to act as a trusted "border
router", implementing the aipowermeme wire protocol and relay, making
...
The ZeroMQ feature in AIPowerMeme Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aipowermemed:
...
For instance:
    ...
    $ aipowermemed -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/aipowermemed.tx.raw \
...
These options can also be provided in aipowermeme.conf.
...
From the perspective of aipowermemed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aipowermemed directly. Furthermore, no information is
...
using. AIPowerMemed appends an up-counting sequence number to each
