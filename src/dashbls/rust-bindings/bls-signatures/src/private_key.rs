...
impl PrivateKey {
    ...
    pub fn generate_aipowermeme(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "aipowermeme_helpers")]
    pub fn generate_aipowermeme_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_aipowermeme(rng))
            .collect()
    }
