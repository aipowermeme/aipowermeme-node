...
const char * const BITCOIN_CONF_FILENAME = "aipowermeme.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //aipowermeme-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\AIPowerMeme
    // macOS: ~/Library/Application Support/AIPowerMeme
    // Unix-like: ~/.aipowermeme
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AIPowerMeme";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/AIPowerMeme";
#else
    // Unix-like
    return pathRet / ".aipowermeme";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty aipowermeme.conf if it does not exist
