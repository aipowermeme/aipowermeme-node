...
def aipowermemehash(s):
    return aipowermeme_hash.getPoWHash(s)
...
# Objects that map to aipowermemed objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(aipowermemehash(r))
            self.hash = aipowermemehash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(aipowermemehash(r))
            self.hash = int(aipowermemehash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in aipowermemed indicates these should be deserialized as blocks
